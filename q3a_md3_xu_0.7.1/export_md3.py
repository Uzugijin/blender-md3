...
class MD3Exporter:
    def __init__(self, context, group_data=None):
        ...
        self.export_frames = self.get_group_frame_range()
    ...
    def get_group_frame_range(self):
        """Get the appropriate frame range for export (group-aware or single-file)"""
        if self.group_data and self.group_data.get('action_strips'):
            # Group export with specific strips
            return self.get_frames_from_strips(self.group_data['action_strips'])
        elif self.modeltype == "static":
            # Static model - just frame 0
            return [0]
        else:
            # Original animated export - full range
            return list(range(self.scene.frame_start, self.scene.frame_end + 1))

    def get_frames_from_strips(self, strip_indices):
        ...
        q3anim_obj = self.get_q3anim_object()
        if not q3anim_obj:
    ...
    def __call__(self, filename):
        if self.group_data:
            ...
            if self.group_data.get('action_strips'):
                self.export_frames = self.get_frames_from_strips(self.group_data['action_strips'])
            else:
        ...
        frames_bin = [self.pack_frame(i) for i in self.export_frames]
