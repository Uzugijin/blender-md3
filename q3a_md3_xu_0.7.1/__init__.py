...
class Q3AnimationConfigProperties(bpy.types.PropertyGroup):
    ...
    modeltype: bpy.props.EnumProperty(
        items=[
            ("assembly", "Multiple Animated", ""),
            ("animated", "Single Animated", ""),
            ("static", "Static", "")
        ],
        name="Type",
        default="assembly",
...
class Q3GenerateTemplate(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if "Map_Bounds" not in bpy.data.objects:
            self.create_bounding_box("Max_Bounds", (102.39, 102.39, 102.39), (0, 0, 0))
        else:
            print("Map_Bounds already exists, skipping...")
...
class ExportMD3(bpy.types.Operator, ExportHelper):
    ...
    def execute(self, context):
        try:
            ...
            from .assembly_map import collect_assembly_groups, get_character_groups
            ...
            if props.skin_enabled and not props.modeltype == "static":
                if character_groups:
